...
export class SvgComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    constructor(
        ...
        this.subscriptionPolygons = svgToolService.polygonPoints$.subscribe(
            points => {
                let idPolygon = points[0].toString();
                let stringPoints = points[1].toString();
    ...
    */
    addSettingsZoom(){
        ...
        console.log("Add Settings Zoom Function ::");
    ...
    doZoomReset(event){
        debugger
        let elementG = this.svgTag.nativeElement.querySelector('g');
        console.log("Elemento :: " + elementG);
    ...
    clickInSvg(event){
        if(this.isDrawing){
            ...
            //else{
                ...
                const objJesus = this.svgTag.nativeElement.querySelector('g');

                let pt = this.svgTag.nativeElement.createSVGPoint();
                pt.x = event.clientX;
                pt.y = event.clientY;
                let newPoint = pt.matrixTransform(objJesus.getScreenCTM().inverse());
                this.pX = newPoint.x;
                this.pY = newPoint.y;
